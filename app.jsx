/* global React, ReactDOM, I, Pill,
   ...
   loadFleetFromStorage, saveFleetToStorage, exportFleetJson */
...
function App() {
  ...
  // Load fleet from localStorage JSON on first mount; fall back to built-in seed data
  const [fleet, setFleet] = React.useState(() => loadFleetFromStorage() || FLEET_INITIAL);
  const [currentId, setCurrentId] = React.useState(() => {
    const stored = loadFleetFromStorage();
    return stored && stored.length > 0 ? stored[0].id : FLEET_INITIAL[0].id;
  });
  const device = fleet.find((d) => d.id === currentId) || fleet[0];

  // Persist fleet to localStorage as JSON whenever it changes
  React.useEffect(() => {
    saveFleetToStorage(fleet);
  }, [fleet]);
    ...
    <>
      <div className={`app ${t.density === "dense" ? "dense" : ""} ${route === "fleet" ? "no-sidebar" : ""}`}>
        ...
        <Topbar route={route} onToggleCli={() => setCliOpen(!cliOpen)} cliOpen={cliOpen}
                ...
                onExportJson={() => exportFleetJson(fleet)}
                device={device}/>
